...
{
    if (x)
        DL_GPIO_setPins(OLED_PORT, OLED_SCL);
    else
        DL_GPIO_clearPins(OLED_PORT, OLED_SCL);
}

static void OLED_W_SDA(uint8_t x)
{
    if (x)
        DL_GPIO_setPins(OLED_PORT, OLED_SDA);
    else
        DL_GPIO_clearPins(OLED_PORT, OLED_SDA);
}
...
{
    OLED_I2C_Start();
    OLED_I2C_WriteByte(0x78);     // 地址
    OLED_I2C_WaitAck();
    OLED_I2C_WriteByte(0x00);     // 写命令
    OLED_I2C_WaitAck();
    OLED_I2C_WriteByte(cmd);      // 命令内容
    OLED_I2C_WaitAck();
    OLED_I2C_Stop();
}
...
void OLED_SetPos(uint8_t x, uint8_t y)
{
    ...
    OLED_WriteCommand(0x10 + ((x >> 4) & 0x0F));    // 列高4位
}
...
void OLED_ShowChar(uint8_t x, uint8_t y, char chr) {
    ...
    OLED_SetPos(x, y);
    for (uint8_t i = 0; i < 8; i++) {
        OLED_WriteData(OLED_F8x16[c][i]);   // 上半部分
    }

    OLED_SetPos(x,y+1);

    for (uint8_t i = 0; i < 8; i++) {
        OLED_WriteData(OLED_F8x16[c][i + 8]);   // 下半部分
    }
}
...
void OLED_ShowNum(uint8_t x, uint8_t y, uint32_t num, uint8_t len)
{
    ...
    for (uint8_t i = 0; i < len; i++) {
        buf[len - i - 1] = num % 10 + '0';
        num /= 10;
    }

    for (uint8_t i = 0; i < len; i++) {
        OLED_ShowChar(x + (i * 8), y, buf[i]);
    }
}
